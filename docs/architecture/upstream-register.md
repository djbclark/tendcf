...
| B-21 | **The augments loader (`def.json`) has the same null-crash root cause**, at three sites in `libpromises/generic_agent.c`: `:454` (`vars`), `:606` (`variables` — note `{"value": null}` is *safe* there, that path has a `NULL_JSON` guard `cmdb.c` lacks) and `:723` (`classes`, NULL into `CheckContextClassmatch()`). The array paths `:749`/`:792`/`:874` are safe because `JsonIteratorNextValueByType(..., skip_null=true)` filters nulls. Found while fixing B-20 | core | *not started* — deliberately not folded into B-20's PR: different file, different feature, different acceptance-test directory (`00_basics/def.json/`) | — | — (Jira only now) | *pending* | — (Jira only now) | *pending* [CFE-4739](https://northerntech.atlassian.net/browse/CFE-4739) |
