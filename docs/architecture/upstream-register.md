...
|---|---|---|---|---|---|---|---|---|
| B-1 | Poll loops count iterations instead of measuring elapsed time, so the termination ladder overshoots ~4.5x on Darwin | core | **done** `26634ac1f` + `943d5371f` | **done** [`fix/exec-timeout-commands`](https://github.com/djbclark/core/tree/fix/exec-timeout-commands) | **done** [#4](https://github.com/djbclark/core/issues/4) | **done** 3-model panel; found 2 defects, both fixed; **withdrew the fail-open claim** | **done** — `security@` 2026-08-17, Gmail id `1a00d22ac0d46c9b`, + correcting follow-up `1a00d44a2758b9ea` | *pending* — not offered as an upstream PR yet |
| B-2 | Descendants not signalled on timeout; grandchild holds the pipe, so `exec_timeout` does not bound wall clock | core | **done** `cb2561584` + `847373cf6` | **done** [`fix/timeout-process-group`](https://github.com/djbclark/core/tree/fix/timeout-process-group) | **done** [#5](https://github.com/djbclark/core/issues/5) | **done** 3-model panel; all three refused the unconditional `setpgid`; regression found and fixed | **done** — `security@` 2026-08-17, Gmail id `1a00d22ac0d46c9b` (same mail as B-1/B-8) | *pending* — not offered as an upstream PR yet |
...
- **RESOLVED 2026-08-17 — Jira works, and every open item is now filed there.**
  ...
  **CFE is fully public and offers no restricted-visibility option** — anonymous
  ...
  | **B-14** JSON parser double-decodes escapes | [CFE-4731](https://northerntech.atlassian.net/browse/CFE-4731) | | |
  ...
  exec_timeout termination half. **CFE-4731 (B-14) was filed 2026-08-17 while
