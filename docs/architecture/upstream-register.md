...
| B-2 | Descendants not signalled on timeout; grandchild holds the pipe, so `exec_timeout` does not bound wall clock | core | **done** `cb2561584` + `847373cf6` | **done** [`fix/timeout-process-group`](https://github.com/djbclark/core/tree/fix/timeout-process-group) | **done** [#5](https://github.com/djbclark/core/issues/5) | **done** 3-model panel; all three refused the unconditional `setpgid`; regression found and fixed | **done** — `security@` 2026-08-17, Gmail id `1a00d22ac0d46c9b` (same mail as B-1/B-8) | *pending* — **blocked on a merge with `#6299`, not on effort.** B-2 touches all three of B-8's files (`cf-agent/verify_exec.c`, `libpromises/timeout.c`, `libpromises/timeout.h`) and **conflicts**: `git merge-tree` rc=1, 10 conflict markers. The two are complementary rather than contradictory — B-8 adds `TIMEOUT_FIRED`/`TIMEOUT_SIGNALLED` outcome reporting, B-2 adds `TIMEOUT_ARMED`/`ClearTimeOut()` and the process-group kill — but they edit adjacent lines of the same `SetTimeOut()`/`TimeOut()`. Resolving it is **new C in a signal path**, so it needs `fable-deep` with `model: 'fable'`. Also note B-2's `ClearTimeOut()` replaces the open-coded `alarm(0); signal(SIGALRM, SIG_DFL)` that B-8 samples its flag *before*, so the merged `ClearTimeOut()` must not clear `TIMEOUT_FIRED`. B-1's `#6300` is **disjoint** from B-8 and needed no such merge | [CFE-4729](https://northerntech.atlassian.net/browse/CFE-4729) |
